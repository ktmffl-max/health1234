...
const workWeight = (k,w) => LIFT[k].start + LIFT[k].inc*(w-1);
...
function liftCard(key,week){
  ...
  return `<section class="lift" style="--accent:${L.accent}">
    <div class="lift-top">
      ...
      <div class="bigw"><b class="num">${fmt(work)}</b><i>kg</i></div>
    </div>
...
function exHTML(e){
  return `<article class="ex"><div class="ex-h">
      ...
      <div class="ex-p"><b class="num">${e.s} × ${esc(e.r)}</b><span class="num">${esc(e.w)}</span></div></div>
...
function dayHTML(d,week){
  ...
  const body = d.groups
    ? d.groups.map(([g,list])=>`<div class="grouphead">${esc(g)}</div>`+list.map(exHTML).join("")).join("")
    : `<div class="grouphead">운동 구성</div>`+d.ex.map(exHTML).join("");
...
function render(){
  ...
  document.getElementById("app").innerHTML = html +
    `<p class="footnote">원본: ${esc(D.source)} · 중량은 시트와 동일한 계산식(MROUND 2.5kg, 웜업 40/60/75/90%)으로 산출됩니다.</p>`;
